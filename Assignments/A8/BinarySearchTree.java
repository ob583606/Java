...
class BinarySearchTree <K extends Comparable<K>, V>  {
	...
	public void insert (K key, V value) {
		if (root==null) {
			...
			try {
				find(key);
				overwrite(null, root, key, value);
			} catch (KeyNotFoundException e) {
				insert(null, root, key, value);
			}
		}
	}
	public void overwrite (BSTNode<K,V> parent, BSTNode<K,V> t, K key, V value) {
		if (t == null) {
			return;
		} else if (key.compareTo(t.key) == 0) {
			t.value = value;
		} else {
			overwrite(t, t.left, key, value);
			overwrite(t, t.right, key, value);
		}
	}
	public void insert(BSTNode<K,V> parent, BSTNode<K,V> t, K key, V value) {
        if (t==null) {
            ...
            if (key.compareTo(t.key) > 0) {
                insert(t, t.right, key, value);
			} else {
                insert(t, t.left, key, value);
			}
        }
    }
	...
	public V findHelp (BSTNode<K,V> t, K key) throws KeyNotFoundException {
		if (t == null) {
		...
		if (t.key.compareTo(key) == 0) {
			...
			return findHelp(t.left, key);
		} else {
			return findHelp(t.right, key);
		}
	}
	/* 	
	...
	public int height() {
		if (root == null) {
			...
			return Math.max(heightHelp(root.left), heightHelp(root.right));
		}
	}
	public int heightHelp(BSTNode<K, V> t) {
		if (t == null) {
			return 0;
		} else {
			return 1 + Math.max(heightHelp(t.left), heightHelp(t.right));
		}
	}
	...
	public List<Entry<K,V>>	entryList() {
		...
		q.addLast(root);
		while (q.size() != 0) {
			BSTNode<K,V> f = q.removeFirst();
			l.add(new Entry<K,V>(f.key, f.value));
			if (f.left != null) {
				q.addLast(f.left);
			}
			if (f.right != null) {
